...
- **推奨 (現在・将来):** `https://chat.dongurihub.com/join/{invite_code}`
- **互換用 (レガシー):** `https://chat.dongurihub.com/add-server?invite={base64_payload}`
...
1. **シンプルで共有しやすい**:
   `https://chat.dongurihub.com/join/abc123` のように短く、チャットやSNSで共有しやすい形式です。
2. **安全性の向上**:
...
## 制限事項：localhost は Public URL に使えません

- **localhost / 127.0.0.1 / 0.0.0.0 の Public URL は保存できません。**
- 共通招待リンクを解決して他のユーザーがサーバーに参加するには、Hub サーバー（および参加するユーザー）からあなたの `CATCHAT_SERVER_PUBLIC_URL` に直接通信できる必要があります。
- そのため、`./setup.sh` は Cloudflare Tunnel、Nginx + HTTPS、Tailscale、または外部から到達できる URL の入力を必須にしています。
...
- **Hub サーバーが一時的にダウンしている、またはネットワーク疎通がない**:
  - **対策**: `CATCHAT_HUB_URL` (通常 `https://chat.dongurihub.com`) にブラウザや curl でアクセスできるか確認してください。
